...
import { getItems, getCurrentUser } from "@/lib/store";
...
export default function Rides() {
  ...
  const refresh = () => {
    const all = getItems<Ride>("rides");
    setRides(all);
    setMyRides(all.filter(r => r.user_id === user.id));
  };
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-4 mb-6 h-12 rounded-2xl bg-secondary p-1">
              <TabsTrigger value="browse" className="rounded-xl data-[state=active]:bg-card text-xs px-1"><Search className="h-4 w-4 mr-1" />Browse</TabsTrigger>
              <TabsTrigger value="offer" className="rounded-xl data-[state=active]:bg-card text-xs px-1"><Plus className="h-4 w-4 mr-1" />Offer</TabsTrigger>
              <TabsTrigger value="my-rides" className="rounded-xl data-[state=active]:bg-card text-xs px-1"><Car className="h-4 w-4 mr-1" />Posted</TabsTrigger>
              <TabsTrigger value="requests" className="rounded-xl data-[state=active]:bg-card text-xs px-1"><Inbox className="h-4 w-4 mr-1" />Requests</TabsTrigger>
            </TabsList>
